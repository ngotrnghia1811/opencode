...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      // mode) tag the user message so `currentModel` ignores it for the next turn.
      ...
      if (switchOverrideExit && Exit.isFailure(switchOverrideExit)) {
        ...
        const suggestions =
          Provider.ModelNotFoundError.isInstance(cause) && cause.suggestions?.length
            ? ` Did you mean: ${cause.suggestions.join(", ")}?`
            : ""
