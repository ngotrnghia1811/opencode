...
function extractSubagentType(tool: string, args: unknown): string | undefined {
...
async function readFileForRule(
  ...
  if (rule.tail_bytes && templated.length > rule.tail_bytes) {
    return `... [reminders: showing last ${rule.tail_bytes} bytes of ${filePath}]\n\n${templated.slice(-rule.tail_bytes)}`
  }
  return templated
}
...
function applyTemplate(text: string, ctx: TemplateCtx): string | undefined {
